...
class MantenimientoRapidoController extends Controller
{
    ...
    /**
     * NUEVO MÉTODO: Notificar al cliente por SMS con Twilio
     */
    public function notificarCliente(Request $request)
    {
        $request->validate([
            'mantenimiento_id' => 'required|integer|exists:cwmantenimientos,id'
        ]);

        $mantenimiento = CWMantenimiento::with(['vehiculo', 'vehiculo.cliente', 'tipos', 'vendedor'])
            ->findOrFail($request->mantenimiento_id);
        ...
        if (!$telefono) {
            return response()->json([
                'success' => false,
                'message' => 'El cliente no tiene número de teléfono registrado. No se puede enviar la notificación.'
            ]);
        }

        // Formatear el número para Twilio
        $telefonoFormateado = $this->formatearNumeroTelefono($telefono);

        // Construir resumen de servicios
        $serviciosRealizados = [];
        foreach ($mantenimiento->tipos as $tipo) {
            $serviciosRealizados[] = $this->getNombreServicio($tipo->tipo);
        }

        if (empty($serviciosRealizados)) {
            $serviciosRealizados[] = $mantenimiento->tipo_mantenimiento ?? 'Mantenimiento general';
        }

        $resumenServicios = implode(', ', array_slice($serviciosRealizados, 0, 3));
        if (count($serviciosRealizados) > 3) {
            $resumenServicios .= ' y más';
        }

        // Preparar mensaje SMS
        $mensaje = $this->prepararMensajeSMS(
            $mantenimiento->vehiculo->cliente->descrip,
            $mantenimiento->vehiculo->marca,
            $mantenimiento->vehiculo->modelo,
            $mantenimiento->vehiculo->identificacion,
            $resumenServicios,
            $mantenimiento->kilometraje,
            $mantenimiento->vendedor->descrip ?? 'nuestro equipo',
            $urlCliente
        );
        ...
        try {
            ...
            Log::info('SMS enviado exitosamente', [
                'mantenimiento_id' => $mantenimiento->id,
                'cliente' => $mantenimiento->vehiculo->cliente->descrip,
    ...
    /**
     * Preparar mensaje SMS para el cliente
     */
    private function prepararMensajeSMS($nombreCliente, $marca, $modelo, $placa, $servicios, $kilometraje, $vendedor, $url)
    {
        $kilometrajeTexto = $kilometraje ? number_format($kilometraje, 0, ',', '.') . ' km' : 'No registrado';

        // Versión corta para SMS (menos de 160 caracteres)
        $mensaje = "¡Hola {$nombreCliente}! ✅ Tu {$marca} {$modelo} ({$placa}) fue atendido.\n";
        $mensaje .= "Servicios: {$servicios}\n";
        $mensaje .= "Kilometraje: {$kilometrajeTexto}\n";
        $mensaje .= "Atendido por: {$vendedor}\n\n";
        $mensaje .= "📱 Ver detalles y fotos:\n{$url}\n\n";
        $mensaje .= "¡Gracias por confiar en nosotros! 🚗";

        // Si el mensaje es muy largo, acortarlo
        if (strlen($mensaje) > 160) {
            $mensaje = "✅ Mantenimiento completado: {$marca} {$modelo} ({$placa})\n";
            $mensaje .= "Servicios: {$servicios}\n";
            $mensaje .= "Kilometraje: {$kilometrajeTexto}\n";
            $mensaje .= "Detalles: {$url}";
        }
     ...
     */
    private function formatearNumeroTelefono($telefono)
    {
        ...
        // Formato para Venezuela (código de país +58)
        if (strlen($telefono) == 10) {
            $telefono = '58' . $telefono;
        } elseif (strlen($telefono) == 11 && substr($telefono, 0, 1) == '0') {
     ...
     */
    private function getNombreServicio($tipo)
    {
        $nombres = [
            'cambio_aceite' => 'Cambio de Aceite',
            'cambio_filtro_aceite' => 'Cambio Filtro Aceite',
            'cambio_filtro_gasolina' => 'Cambio Filtro Gasolina',
            'cambio_filtro_aire' => 'Cambio Filtro Aire',
            'mantenimiento_inyectores' => 'Mantenimiento Inyectores',
            'bateria' => 'Batería',
